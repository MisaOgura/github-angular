...
module.exports = function CohortInfo() {
  ...
  CohortInfo.prototype.requestToGitHub = function() {
    ...
    var options = {
      url: 'https://api.github.com/users/' + '?client_id=05d7cca7933ca20904c9&client_secret=a33d9a9dd9eb1cf1e1025c1a012a724961c2ce24',
      headers: {
        'User-Agent': 'request'
      }
    };
    ...
    function _listCohort() {
      fs.readFile('feb16Cohort.txt', function(err, data) {
        if (err) console.log(err);
        cohort = data.toString().split('\n');
        cohort.pop();
        console.log('inside _listCohort');
        console.log(cohort);
        return cohort;
      });
    }
